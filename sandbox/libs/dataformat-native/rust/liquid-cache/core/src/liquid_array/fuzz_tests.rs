...
mod fuzz {
    ...
    fn assert_filter_matches(original: &ArrayRef, liquid_filtered: &ArrayRef, mask: &BooleanBuffer) {
    ...
    fn opt_i32_vec(max_len: usize) -> impl Strategy<Value = Vec<Option<i32>>> {
        proptest::collection::vec(
            proptest::option::weighted(0.9, any::<i32>()),
            0..=max_len,
        )
    }
    ...
    fn opt_u64_vec(max_len: usize) -> impl Strategy<Value = Vec<Option<u64>>> {
        proptest::collection::vec(
            proptest::option::weighted(0.9, any::<u64>()),
            0..=max_len,
        )
    }
